...
enum GeminiChapters {
    ...
    static func generate(timedLines: [(startMs: Int64, text: String)]) async -> [Chapter]? {
        ...
        for line in trimmed {
            ...
            lines += "[\(String(format: "%02d:%02d", mm, ss))] \(clean)\n"
        }

        let system = """
        You generate Loom-style chapters for a meeting transcript.
        Output ONLY a JSON object with a `chapters` array. Each
        chapter has `start_ms` (integer milliseconds, must match the
        timestamp of an actual line in the transcript) and `title`
        (3–7 words describing the topic of that segment in the
