...
### Changed
- Reorganized Settings: General holds the everyday toggles (recording equalizer, theme, notifications, auto-transcribe, auto-summary, auto-chapters); Advanced holds capture and device settings (microphone, screen video, recordings folder, launch at login, diagnostics). Advanced is now visible without signing in.
- Paid plans no longer pre-download the on-device model in the background after you upgrade. It is opt-in now, from the Offline transcription setting.
