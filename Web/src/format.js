...
export function formatDate(ms, lang = "ru") {
    ...
    if (isYesterday)
        return lang === "ru" ? `Вчера, ${time}` : `Yesterday, ${time}`;
...
export function dateBucket(ms, lang = "ru") {
    const d = new Date(ms);
    const now = new Date();
    ...
    if (diffDays < 7)
        return lang === "ru" ? "НА ЭТОЙ НЕДЕЛЕ" : "THIS WEEK";
    if (diffDays < 30)
        return lang === "ru" ? "В ЭТОМ МЕСЯЦЕ" : "THIS MONTH";
