...
export function ChaptersPane({ detail, onSeek, onToast, t }: Props) {
      ...
      <div className="summary-content ovsb-scroll" ref={scrollRef}>
        {filtered.map((c, i) => (
          <div key={i} className="chapter-block">
            {/* Time pill — h3-sized (matches Summary section headings)
                and the only thing that lights up on hover, so it
                reads as the click target. */}
            <button
              type="button"
              className="chapter-time"
              onClick={() => onSeek(Math.max(0, c.startMs / 1000))}
              aria-label={fmtTime(c.startMs)}
            >
              {fmtTime(c.startMs)}
            </button>
            <p className="md-p chapter-title">{c.title}</p>
          </div>
        ))}
      </div>
